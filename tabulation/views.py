...
def Get_HeartBeat(time_stamps, records):
    cur = connection.cursor()
    command = "SELECT time_stamp,value from dashboard_heartbeatdata where time_stamp between  FROM_UNIXTIME({time})-interval 1 minute and FROM_UNIXTIME({time}) limit {records}".format(time=time_stamps, records=records)
    cur.execute(command)
    return cur.fetchall()
